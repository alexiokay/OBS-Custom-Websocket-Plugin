...
namespace vorti::concepts {
    ...
    // Numeric concept for width/height parameters
    template<typename T>
    concept Numeric = std::integral<T> || std::floating_point<T>;

    // Container concept for collections
    template<typename T>
    ...
    // JSON-serializable concept
    template<typename T>
    concept JsonSerializable = requires(T t) {
        { nlohmann::json(t) } -> std::convertible_to<nlohmann::json>;
    };

    // OBS source concept for type safety
    template<typename T>
    concept ObsSourcePtr = std::same_as<T, obs_source_t*>;
}

// C++20 consteval helper functions for compile-time constants
namespace vorti::constants {
    // Compile-time frame rate calculation
    consteval long calculate_fps_interval(int fps) {
        return 1000 / fps;
    }
    
    // Compile-time string length validation
    consteval bool validate_string_length(std::string_view str, size_t max_length) {
        return str.length() <= max_length;
    }
    
    // Compile-time action name validation
    consteval bool is_valid_action_name(std::string_view name) {
        return !name.empty() && name.find_first_not_of("abcdefghijklmnopqrstuvwxyz0123456789_") == std::string_view::npos;
    }
}
...
namespace vorti
{
    namespace applets
    {
        namespace obs_plugin
        {
            ...
            static const long m_update_interval = vorti::constants::calculate_fps_interval(1);  // 1 fps
            ...
            // clang-format off
            const std::string s_integration_name            = "VortiDeck OBS Plugin";
            const std::string s_integration_author          = "VortiDeck";
            const std::string s_integration_description     = "A VortiDeck plugin for Open Broadcaster Software, exposing additional actions to VortiDeck desktop app.";
            const std::string s_integration_identifier      = "vortideck_obs_plugin";
